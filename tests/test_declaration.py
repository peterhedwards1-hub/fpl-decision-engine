...
def test_a_settings_change_moves_the_digest(tmp_path) -> None:
    ...
    base = ModelDeclaration(
        model_config=CHALLENGER, team_strength_settings=TeamStrengthSettings()
    )
    for field, value in (
        ("prior_matches_intact_squad", 20.0),
        ("current_half_life_gameweeks", 4.0),
...
def test_a_declaration_round_trips() -> None:
    original = ModelDeclaration(
        ...
        team_strength_settings=TeamStrengthSettings(prior_matches_intact_squad=9.0),
