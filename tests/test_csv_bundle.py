...
def test_csv_loader_requires_player_season_references_for_stats_and_observations(tmp_path) -> None:
    _write_minimum_bundle(tmp_path)
    (tmp_path / "player_fixture_stats.csv").write_text(
        "source_player_id,source_fixture_id,minutes\n999,5001,90\n", encoding="utf-8"
    )
    with pytest.raises(CsvBundleError, match="player_seasons.csv"):
        load_csv_bundle(tmp_path, SEASON)

    _write_minimum_bundle(tmp_path)
    (tmp_path / "player_gameweek_snapshots.csv").write_text(
        "source_player_id,gameweek_number,price_tenths\n999,1,75\n", encoding="utf-8"
    )
    with pytest.raises(CsvBundleError, match="player_seasons.csv"):
        load_csv_bundle(tmp_path, SEASON)


def test_csv_loader_rejects_blank_required_values_and_invalid_timing_combinations(tmp_path) -> None:
    _write_minimum_bundle(tmp_path)
    (tmp_path / "teams.csv").write_text(
        "source_team_id,name,short_name\n1,,NTH\n2,South City,STH\n", encoding="utf-8"
    )
    with pytest.raises(CsvBundleError, match="teams.csv.*name.*required"):
        load_csv_bundle(tmp_path, SEASON)

    _write_minimum_bundle(tmp_path)
    (tmp_path / "player_gameweek_snapshots.csv").write_text(
        "source_player_id,gameweek_number,price_tenths,timing_quality,observed_on\n"
        "101,1,75,date_only,\n",
        encoding="utf-8",
    )
    with pytest.raises(CsvBundleError, match="date_only.*observed_on"):
        load_csv_bundle(tmp_path, SEASON)

    _write_minimum_bundle(tmp_path)
    (tmp_path / "player_gameweek_snapshots.csv").write_text(
        "source_player_id,gameweek_number,price_tenths,timing_quality,observed_on\n"
        "101,1,75,date_only,2025-08-15\n",
        encoding="utf-8",
    )
    bundle = load_csv_bundle(tmp_path, SEASON)
    assert bundle.gameweek_snapshots[0].observed_on.isoformat() == "2025-08-15"
