...
def create_news_projection_pair(
    ...
    *,
    ...
    if pending:
        raise WorkflowError(
            f"Cannot generate final news projections with {pending} "
            "pending evidence item(s)"
        )
    ...
    if not evidence:
        raise WorkflowError(
            "No current accepted player-level minutes adjustments are available"
        )
    ...
    for item in evidence:
        ...
        if player_id not in expected_minutes:
            raise WorkflowError(
                f"Accepted news player {player_id!r} is absent from "
                "the pre-news projection"
            )
        ...
        rationale.setdefault(player_id, []).append(
            f"news:{evidence_id} {item['summary']} "
            f"({item['rationale']})"
        )
    ...
    if (
        pre_news.projection_run_id is None
        or post_news.projection_run_id is None
    ):
    ...
    return NewsProjectionPair(
        pair_id=pair_id,
        pre_news_projection_run_id=pre_news.projection_run_id,
...
def evaluate_news_projection_pair(
    ...
    *,
    ...
    if unfinished:
        raise WorkflowError(
            f"Cannot evaluate news projections while {unfinished} "
            "fixture(s) are unfinished"
        )
...
def _mae(rows: list, expected: str, actual: str) -> float:
    return round(
        sum(abs(float(row[actual]) - float(row[expected])) for row in rows)
        / len(rows),
        4,
    )
